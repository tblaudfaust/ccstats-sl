...
Qcell SIP     ──┴─ UDP 5060 ─► FreePBX 17 / Asterisk ◄─ WSS 8089 ┘
                               ...
                               │  Apache :443  pbx.DOMAIN  (admin UI, UCP)
                               │  Apache :443  crm.DOMAIN  ─► Docker: Odoo 18 + Postgres 16
```
...
4. In your DNS create two A records pointing at the VPS IP: `pbx.<yourdomain>` and `crm.<yourdomain>`.
...
7. Tell me: the VPS IP, the two hostnames, your office public IP, and the Let's Encrypt email.
...
| 2 | `scripts/02-odoo-stack.sh` | Docker, Odoo 18 + Postgres, OCA addons, Apache vhost + TLS for crm.DOMAIN |
